...
- Treat `make doctor` as an auxiliary host or runtime readiness check when it is present; it is not part of the required verify contract.
...
- From a `rune` shell, generated shared tool invocations bridge into codex-owned tooling automatically when needed.
